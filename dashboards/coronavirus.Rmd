...
all_data <- download_merged_data(silent = TRUE, cached = TRUE)

# first_case_date <- all_data %>%
#   group_by(country) %>%
#   summarise(first_conf = min(date))
# 
# all_data %<>%
#   left_join(first_case_date) %>%
#   mutate(first_conf = as.numeric(date - first_conf)) %>%
#   select(date, first_conf, everything())
```
...
selectizeInput("country",
  ...
  choices = sort(unique(all_data$country)),
  selected = c(
    "Italy", "Spain",
...
```{r}
# TODO: last update above reflects date of server, not of publication.
...
_Last update_:   
**`r format(Sys.Date(), "%B %d %Y at %H:%M GMT")`**. 
...
t1 <- all_data %>%
  ...
  ggplot(aes(reorder(country, confirmed),
    confirmed,
    fill = confirmed
  )) +
  ...
  coord_flip(ylim = c(NA, max(all_data$confirmed) * 1.2), expand = TRUE) +
  labs(
...
t2 <- all_data %>%
  ...
  coord_flip(ylim = c(NA, max(all_data$cum_dead) * 1.2), expand = TRUE) +
  labs(
...
# all_data %>% 
#   filter(date == max(date)) %>% 
#   select(country, confirmed, cum_dead) %>% 
#   top_n(10, confirmed) %>% 
#   pivot_longer(confirmed:cum_dead) %>% 
...
renderValueBox({
  max_conf <- df() %>%
    filter(date == min(input$daterange[2], max(date)-2)) %>%
...
```{r}
# TODO: here and above replace to date to two days ago
renderValueBox({
  all_conf <- all_data %>%
    filter(date == min(input$daterange[2], max(date)-2)) %>%
    summarise(confirmed = sum(confirmed, na.rm = TRUE)) %>%
...
renderPlot({
  cumplot <- df() %>%
    mutate(end_label = ifelse(date == max(date), glue("{country}: {scales::comma(confirmed)}"), NA))

  if (input$startconf) {
    cumplot %<>% ggplot(aes(first_conf, confirmed, color = country))
  } else {
    cumplot %<>% ggplot(aes(date, confirmed, color = country))
  }
  ...
  cumplot <- cumplot +
    ...
    labs(caption = glue("Data last updated on {max(df()$date)}"))
...
renderPlot({
  deathplot <- df() %>%
    mutate(end_label = ifelse(date == max(date), glue("{country}: {scales::comma(cum_dead)}"), NA))

  if (input$startconf) {
    deathplot %<>% ggplot(aes(first_conf, cum_dead, color = country))
  } else {
    deathplot %<>% ggplot(aes(date, cum_dead, color = country))
  }
...
renderPlot({
  recplot <- df() %>%
    mutate(end_label = ifelse(date == max(date), glue("{country}: {scales::comma(cum_heal)}"), NA))

  if (input$startconf) {
    recplot %<>% ggplot(aes(first_conf, cum_heal, color = country))
  } else {
    recplot %<>% ggplot(aes(date, cum_heal, color = country))
  }
...
df_speed <- reactive({
  df() %>%
    ...
    group_by(country) %>%
    mutate(
      cases_confirm = confirmed - lag(confirmed),
